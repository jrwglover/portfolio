        ...
        leaves: [
          {
            ...
            description:
              'One chart over the whole curve set. Pick curves, then read them as discrete forwards (the 3M or 6M rate a FRA or future actually pays), as zero rates, or as raw discount factors, out to 2.5, 10 or 30 years. Each curve is solved in the domain its own instruments pin: OIS strips on zero rates, the meeting-dated and IMM curves as flat forwards between policy or IMM dates joined to a min-curvature forward, EURIBOR on discrete 6M forwards, and EUR/USD as an implied zero curve against USD collateral. Those 6M forwards are reconstructed as the smoothest instantaneous forward that reproduces every one exactly - minimum curvature under interval-average constraints, since a FRA pins the average of the forward over its window and never the forward at a point.',
            techBadges: ['C++', 'QuantLib', 'GlobalBootstrap', 'CUDA'],
