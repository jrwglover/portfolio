...
import type { TopicLeaf } from '../../config/topics';

export default function TopicCard({ leaf }: { leaf: TopicLeaf }) {
  const isLive = leaf.status === 'live';

  return (
    <Link to={`/learn/${leaf.slug}`}>
      <article className="glass-card rounded-lg p-6 h-full flex flex-col cursor-pointer">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs" style={{ color: 'var(--text-dim)' }}>
            {leaf.techBadges[0]}
            {leaf.techBadges[1] ? ` + ${leaf.techBadges[1]}` : ''}
          </span>
        </div>
